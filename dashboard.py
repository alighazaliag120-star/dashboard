...
# Inisialisasi status login di memory Streamlit
...
# --- HALAMAN LOGIN (Muncul kalau belum login) ---
if not st.session_state['login_sukses']:
    ...
    # Bikin tampilan ke tengah pakai kolom biar rapi
...
# --- HALAMAN UTAMA (Muncul HANYA kalau sudah login) ---
else:
    # Kolom untuk header dan tombol logout di kanan atas
    ...
    with header_col2:
        ...
        if st.button("Keluar / Logout", use_container_width=True):
            st.session_state['login_sukses'] = False
            st.rerun() # Refresh halaman kembali ke login

    st.success("Berhasil masuk! Selamat bekerja.")
    st.markdown("---")

    # =================================================================
    # SIDEBAR NAVIGATION (SEMUA MASUK KE DALAM BLOK 'ELSE')
    # =================================================================
    with st.sidebar:
        st.title("Main Menu")
